...
netzfc = torch.load('saved_netz_join.py')
...
def test_var_noise():
  ...
  #return (hits/len(testing_data)), (hits_fc/len(testing_data)), (hits_won/len(testing_data)), (hits_algo/len(testing_data))
...
testing_data = get_train_dataset("../data/test_join.txt", "../data/test_inf.txt", 1)
while (b < 0.51):
  print(b)
  if(b == 0): a = 0
  else: a = 10**(-5)
  #net, fc, won, algo = test_var_noise()
  fc = test_var_noise()
  #acc.append(net)
  acc_fc.append(fc)
  #acc_won.append(won)
...
plt.legend()
plt.savefig('../results/Accuracy_Noise_Join.png')
plt.show()
